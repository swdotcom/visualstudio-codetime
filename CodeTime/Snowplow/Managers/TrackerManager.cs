    ...
    class TrackerManager
    {
        ...
        public async Task initializeTracker()
        {
            if (!initialized)
            {
                ...
                HttpResponseMessage resp = await HttpManager.MetricsRequest(System.Net.Http.HttpMethod.Get, "/plugins/config", null);

                // if (resp.ok && resp.responseData != null)
                if (HttpManager.IsOk(resp))
                {
                    try
                    {
                        // get the json data
                    ...
                    {
                        LogManager.Error("Error initializing snowplow", ex);
                    }
                }
            }
        }
