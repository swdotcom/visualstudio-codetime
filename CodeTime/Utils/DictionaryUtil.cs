...
namespace CodeTime
{
    class DictionaryUtil
    {
        ...
        public static CodeTimeProject ConvertObjectToProject(IDictionary<string, object> dict)
        {
            try
            {
                CodeTimeProject proj = JsonConvert.DeserializeObject<CodeTimeProject>(dict["project"].ToString());
                return proj;
            }
            catch (Exception) { }

            return new CodeTimeProject("Unnamed", "Untitled");
        }
